...
#initialize tkinter
root = Tk()
root.title('Our Player')
root.iconbitmap('F:\Python\Player stock\home.png')
...
cursor.execute("drop table if exists playlist1;")
cursor.execute("CREATE TABLE playlist1 (song varchar(100), url varchar(150));")
...
def openNewWindow2(): 
	...
	def save():
		...
		#q2 =
		#cursor.execute(q1)
		cursor.execute("CREATE TABLE "+pname.get()+"(song varchar(100),url varchar(100));")
		cursor.execute("INSERT INTO "+ pname.get() +" SELECT * FROM playlist1;")
		connection.commit()
		newWindow2.destroy()
...
def main(music_name):
	...
	#s="use Music_Player;"
	#cursor.execute("INSERT INTO table1 VALUES(%s %s)",(song,clip2))
	#print("INSERT INTO table1 VALUES(%s %s)",(song,clip2))
	cursor.execute("INSERT INTO playlist1 VALUES"+"('"+song+"','"+clip2+"')")
	connection.commit()
	return song
...
def search(event):
	...
	cursor.execute("SELECT url from " + v.get()+" where song='"+music_name +"';")
	url = cursor.fetchone()
...
back_btn_img = PhotoImage(file='F:\Python\Player stock\previous.png')
forward_btn_img =  PhotoImage(file='F:\Python\Player stock\pp.png')
play_btn_img =  PhotoImage(file='F:\Python\Player stock\play.png')
pause_btn_img =  PhotoImage(file='F:\Python\Player stock\pause.png')
stop_btn_img =  PhotoImage(file='F:\Python\Player stock\home.png')
